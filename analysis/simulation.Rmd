...
fig_3 <- simulation_output %>% 
  mutate(percent_daughters = 100 * (1 + SR_bias_in_hetero_males)/2) %>%
  ggplot(aes(k, percent_daughters, fill = prop_SD)) +
  geom_blank() +
  geom_raster() + 
  ...
  labs(x = "Strength of segregation distortion (K)", 
       y = "Offspring sex ratio when father is an SD heterozygote (% daughters)") +
